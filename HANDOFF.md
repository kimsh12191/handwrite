...
`reports/taxonomy_v2.md`가 전체 설계다. `taxonomy_v1.md`는 그중 측정된
부분(사실상 3개)이라 공간으로는 한참 모자라며, v1만 보면 안 된다.

v2는 항목마다 `[측정]` / `[문헌]` / `[설계]` 등급을 단다. 근거 없는 축을
지우지 않고 등급을 달아 남기는 이유는, 지우면 다음 사람이 그 층이 필요 없다고
오해하기 때문이다.

v1에 통째로 빠져 있었고 v2가 채운 것 중 중요한 둘:

- **변동 구조(L9).** 분산 자체가 개인 파라미터다. 평균만 모델링하면 같은
  글자가 항상 같아지고, 그것이 폰트·GAN이 즉시 티 나는 이유다.
  파라미터 수가 사실상 두 배가 된다.
- **이산 축(L4).** 이체·획순·획수는 연속 최적화로 원리적으로 넘어갈 수 없다.
  이 공간은 순수 연속이 아니다.

## 다음 병목 하나

**자모 형태 모드를 아직 못 구한다.** 지금까지는 전부 음절 전체의 잉크맵
비교다. 그래서 4번의 잔차가 무엇 때문인지 좁힐 수 없고, 자모 단위 shape 분포도,
§6의 primitive 재사용 질문도 열리지 않는다.

앞서 이 병목을 "자모 분할"이라고 적었는데 절반만 맞았다. 필요한 것은 분할이
아니라 **골격 위의 점 대응(correspondence)** 이고, 분할은 그 수단이다.
`feature → PCA → 파라미터 축`이라는 절차는 사실 PDM/Active Shape Model
(Cootes & Taylor 1995)의 레시피인데, 거기에는 빠뜨린 필수 단계가 있다.

```text
골격 추출 → 점 대응 → Procrustes 정렬 → PCA
```

대응 없이 PCA를 돌리면 분산이 형태가 아니라 어긋남에 쓰인다. 위 2번에서 측정된
증상(상위 성분이 전부 획이 밀린 모드)이 정확히 그것이고, 교과서적 실패 모드다.

**판정 기준은 이미 손에 있다.** 대응을 붙인 뒤 `shape_space.py`를 다시 돌려서
28성분이 크게 떨어지지 않으면 대응이 실패한 것이다. 눈금은
`basis_control.py`가 선언 4-DOF를 4성분으로 읽는 것으로 검증돼 있다.
